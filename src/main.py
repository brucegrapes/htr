...
def main():
    ...
    parser.add_argument('--mode', choices=['train', 'validate', 'infer', 'plag'], default='infer')
    ...
    # infer text on test image
        ...
        firstFile = input('Enter the file1 path')
        model = Model(list(open(FilePaths.fn_char_list).read()), decoder_type, must_restore=True, dump=args.dump)
        text = getText(model,firstFile)
        name_of_file = 'plag'
        completeName = os.path.join(FilePaths.save_path, name_of_file + ".txt")
        print('Plag', text)
